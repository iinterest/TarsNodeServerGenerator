...
exports.listen = () => {
    ...
    process.on('uncaughtException', err => {
        logger.error.info('uncaughtException at:', err.message + '\n' + err.stack)
    })
    ...
    process.on('unhandledRejection', (reason, p) => {
        logger.error.info('Unhandled Rejection at: Promise ', p, ' reason: ', reason)
    })
    ...
    process.on('rejectionHandled', (reason, p) => {
        logger.error.info('Rejection Unhandled at: Promise ', p, ' reason: ', reason)
    })

    /** 任何时候Node.js发出进程告警，都会触发'warning'事件。 */
    process.on('warning', warning => {
        logger.error.info('warning on:', warning.name, warning.message, warning.stack)
    })
